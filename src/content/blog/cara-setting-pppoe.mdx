...
Ini dikarenakan PPPoE menggunakan protokol `point to point` sehingga tidak ada lagi traffic broadcast ARP. Ini dapat mempercepat jaringan secara signifikan ketika jumlah device yang terkoneksi di suatu jaringan fisik sudah banyak.
...
Kemsuk menu PPPudian Kita juga punya mikrotik yang berfungsi sebagai PPPoE client. Mikrotik ini disambungkan ke switch di interface Ether2
...
#### 4. Pilih PPPoE Interface 
Kita pilih interface menuju PPPoE server kita. Sesuai topologi kita, PPPoE server itu ada di interface ether2.
